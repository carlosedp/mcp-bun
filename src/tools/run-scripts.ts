...
export function registerScriptTools(server: McpServer): void {
  // Tool to run a Bun script with optimizations
  server.tool(
    "run-bun-script-file",
    "Execute a JavaScript/TypeScript script file with Bun runtime and optimizations",
    {
      scriptPath: z
        .string()
        .describe("Path to the JavaScript/TypeScript script to execute"),
      bunArgs: z
        .array(z.string())
        .optional()
        .describe(
          "Optional arguments to pass to the Bun executable (e.g., --smol, --hot)"
        ),
      args: z
        .array(z.string())
        .optional()
      ...
      cwd: z
        .string()
        .optional()
        .describe("Directory to run the script in (current working directory)"),
      timeout: z
        .number()
        .optional()
        .describe("Timeout in milliseconds after which the process is killed"),
    },
    async ({ scriptPath, bunArgs = [], args = [], stdin, cwd, timeout }) => {
      try {
        ...
        const bunArgsString = bunArgs.length > 0 ? bunArgs.join(" ") + " " : "";
        ...
        const execOptions: ExecOptionsWithInput = {
          cwd: workingDir,
          timeout: timeout || 60000, // 1 minute default timeout
        };
  ...
  server.tool(
    ...
    "Execute a Node.js script file locally",
    {
      scriptPath: z
        .string()
        .describe(
          "Path to the Node.js script to execute, this should be present on disk"
        ),
      nodeArgs: z
        .array(z.string())
        .optional()
        .describe(
          "Optional arguments to pass to the Node.js executable itselfm, like --test"
        ),
      args: z
        .array(z.string())
        .optional()
  ...
  server.tool(
    ...
    "Execute JavaScript code directly with Node.js eval. Optionally specify a directory to execute in.",
    {
      ...
      evalDirectory: z
        ...
        .optional()
        .describe(
          "Directory to execute the code in (must be an allowed directory)"
        ),
      stdin: z
        .string()
        .optional()
