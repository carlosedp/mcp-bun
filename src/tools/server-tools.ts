...
export function registerServerTools(server: McpServer): void {
  // Tool to start a Bun server in the background
  server.tool(
    "start-bun-server",
    "Start a Bun server that continues running in the background with optimizations",
    {
      scriptPath: z
        .string()
        .describe("Path to the JavaScript/TypeScript server script to execute"),
      cwd: z.string().describe("Directory to run the server in"),
      serverName: z
        .string()
        .optional()
        .describe(
          "Optional friendly name for the server (defaults to filename)"
        ),
      bunArgs: z
        .array(z.string())
        .optional()
        .describe(
          "Optional arguments to pass to the Bun executable itself (e.g., --watch, --hot)"
        ),
      args: z
        .array(z.string())
        .optional()
        .describe("Optional arguments to pass to the server script"),
      optimizations: z
        .object({
          hot: z.boolean().optional().describe("Enable hot reloading"),
          watch: z.boolean().optional().describe("Enable file watching"),
          smol: z
            .boolean()
            .optional()
            .describe("Use smol mode for memory optimization"),
          minify: z.boolean().optional().describe("Enable minification"),
        })
        .optional()
        .describe("Bun optimization options"),
    },
    async ({
      scriptPath,
      serverName,
      ...
      try {
        ...
        // Build optimized bun arguments
        const optimizedArgs = [...bunArgs];

        if (optimizations.hot) {
          optimizedArgs.push("--hot");
        }
        if (optimizations.watch) {
          optimizedArgs.push("--watch");
        }
        if (optimizations.smol) {
          optimizedArgs.push("--smol");
        }
        if (optimizations.minify) {
          optimizedArgs.push("--minify");
        }

        // Format command for permission request
        const bunArgsString =
          optimizedArgs.length > 0 ? optimizedArgs.join(" ") + " " : "";
        const argsString = args.length > 0 ? " " + args.join(" ") : "";
        const command = `bun ${bunArgsString}${absPath}${argsString}`;

        // Get working directory
        const workingDir = path.resolve(cwd);

        // Generate a server name if not provided
        const displayName = serverName || `bun-${path.basename(absPath)}`;

        // Ask for permission
        const permissionMessage = `Start Bun server: ${displayName}\nCommand: ${command}\nWorking directory: ${workingDir}`;
        ...
        // If we have a selected Bun version, get its bin path
        let bunBin = "bun";
        const selectedBunVersion = getSelectedBunVersion();
        if (selectedBunVersion) {
          try {
            // For Bun, we might use a version manager or specific installation path
            const { stdout } = await execAsync(`which bun`);
            bunBin = stdout.trim();
          } catch (error) {
            // Fallback to default bun if version selection fails
            console.warn("Could not find specific Bun version, using default");
          }
        }
        ...
        // Capture output with Bun-specific markers
        serverProcess.stdout.on("data", (data) => {
          const output = data.toString();
          logs.push(`[bun:stdout] ${output}`);
        ...
        return {
          content: [
            {
              type: "text" as const,
              text: `Bun server started successfully!\n\nServer ID: ${serverId}\nName: ${displayName}\nPID: ${
                serverProcess.pid
              }\nCommand: ${command}\nWorking directory: ${workingDir}\nOptimizations: ${
        ...
        return {
          ...
          content: [
            {
              type: "text" as const,
              text: `Error starting Bun server: ${errorMessage}`,
            },
          ],
        };
      }
  ...
  server.tool(
    ...
    "Start a Node.js server that continues running in the background",
    {
      scriptPath: z
        .string()
        .describe("Path to the Node.js server script to execute"),
      cwd: z.string().describe("Directory to run the server in"),
      serverName: z
        .string()
        .optional()
      ...
      nodeArgs: z
        ...
        .optional()
        .describe(
          "Optional arguments to pass to the Node.js executable itself"
        ),
      args: z
        .array(z.string())
        .optional()
    ...
    async ({ scriptPath, serverName, nodeArgs = [], args = [], cwd }) => {
      try {
          ...
          try {
            ...
            return {
              ...
              content: [
                {
                  type: "text" as const,
                  text: `Error getting Node.js binary path: ${
                    error instanceof Error ? error.message : String(error)
                  }`,
                },
              ],
        ...
        // Start the server as a detached process
        const serverProcess = spawn(nodeBin, [...nodeArgs, absPath, ...args], {
          cwd: workingDir,
          env,
          stdio: ["ignore", "pipe", "pipe"],
  ...
  server.tool(
    ...
    "List all running Node.js servers started via MCP",
    {
      showLogs: z
        .boolean()
        .optional()
        .describe("Whether to include recent logs (default: false)"),
      serverId: z
        .string()
        .optional()
        .describe("Optional server ID to get details for a specific server"),
    },
      ...
      try {
        ...
        if (serverId) {
          ...
          const uptime = Math.floor(
            (new Date().getTime() - server.startTime.getTime()) / 1000
          );
          ...
          serverDetail += `Status: ${
            server.exitCode === null
              ? "Running"
              : `Exited with code ${server.exitCode}`
          }\n`;
  ...
  server.tool(
    ...
    "Stop a running Node.js server",
    {
      ...
      force: z
        ...
        .optional()
        .describe(
          "Whether to force kill the server (SIGKILL instead of SIGTERM)"
        ),
    },
    async ({ serverId, force = false }) => {
      try {
        ...
        // Ask for permission
        const permissionMessage = `Stop server: ${serverInfo.name} (PID: ${
          serverInfo.pid
        })${force ? " with force" : ""}`;
        const permitted = await askPermission(permissionMessage);
  ...
  server.tool(
    ...
    "Get the last N lines of logs from a running server",
    {
      ...
      filter: z
        .string()
        .optional()
        .describe("Optional string to filter logs (case-insensitive)"),
      stdout: z
        .boolean()
        .optional()
        .describe("Show stdout logs (default: true)"),
      stderr: z
        .boolean()
        .optional()
        .describe("Show stderr logs (default: true)"),
    },
    async ({ serverId, lines = 50, filter, stdout = true, stderr = true }) => {
      try {
        ...
        if (filteredLogs.length === 0) {
          ...
          if (!stdout && !stderr)
            reason += " (both stdout and stderr are disabled)";
        ...
        // Build the response
        const statusInfo =
          serverInfo.exitCode === null
            ? "Running"
            : `Exited with code ${serverInfo.exitCode}`;
